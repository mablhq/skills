...
| [`mabl-convention-guide`](plugins/mabl/skills/mabl-convention-guide/SKILL.md) | Work out how a workspace names and organizes things and write it up for new joiners. Reads the stated convention out of agent instructions first, infers the observed pattern from the tests second, and reports where the two disagree — with an explicit note on what it could and could not enumerate. |
