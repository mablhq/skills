...
| [`mabl-compare-versions`](plugins/mabl/skills/mabl-compare-versions/SKILL.md) | Say what changed between two versions of a test or reusable flow, or between two of them, without telling you what to think about it. Reports steps added, removed, changed and moved; assertion counts by type; and the changes that keep a test the same size while it proves less — a strict assertion swapped for a looser one, an expected value emptied, a step disabled, a date baked in. Works over the mabl MCP server or the CLI, and covers reusable flows too. The classification is the output; the verdict stays with whoever asked for the change. |
