...
| [`bulk-manage-labels`](plugins/mabl/skills/bulk-manage-labels/SKILL.md) | Clean up label drift across a whole workspace, safely. Sorts every family into cosmetic (case variants, which mabl already treats as one label), functional (genuinely different strings that split a suite), and ambiguous (near-duplicates that might be separate concepts) — then fixes the functional ones, reports the cosmetic ones, and asks about the ambiguous ones instead of merging them. |
