...
| [`mabl-verify-change`](plugins/mabl/skills/mabl-verify-change/SKILL.md) | Prove a change to a test actually fixed it, instead of trusting the green run. Diffs against the version before the change to catch a pass bought by deleting a check, re-runs it isolated on its branch using the run ids the trigger hands back, and needs more than one clean run before calling an intermittent test fixed. Reports verified, not verified, or verified-but-not-green — and never edits or merges. |
