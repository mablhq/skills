...
| [`mabl-plan-triage`](plugins/mabl/skills/mabl-plan-triage/SKILL.md) | A plan run came back with failures — get one answer for all of them instead of one investigation per test. Groups failures by shared cause, checks each test's own history to tell flaky from broken from long-broken, separates an application defect from a test problem, and reports what deserves a person first. Hands the shortlist to `mabl-debug` rather than opening traces itself. |
