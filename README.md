...
| [`mabl-test-run`](plugins/mabl/skills/mabl-test-run/SKILL.md) | Start a run of the test, set of tests, plan, or deployment you already picked, then report what it actually did. Applies the overrides you asked for — application, environment, deployment, URL, credentials, DataTable scenario, mabl branch, browsers — runs locally or in the cloud, and publishes local results into the app when you want them there. A launch response is a dispatch, not a result, so it polls every id it started to a terminal state and reports the outcomes and the cost, with anything it could not see reported as unverified rather than folded into a pass. It never picks what to run and never rules on the result. |
