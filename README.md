...
| [`mabl-plan-create`](plugins/mabl/skills/mabl-plan-create/SKILL.md) | Build a plan — a named set of existing tests that runs against one app in one environment — and change one you already have: add or remove tests, append a stage, relabel, swap credentials, pause it. Reads back what actually saved, because the API drops test ids it doesn't recognise without saying so, and asks before anything that quietly takes tests out of CI. |
