...
| [`mabl-convention-guide`](plugins/mabl/skills/mabl-convention-guide/SKILL.md) | Review a workspace for the conventions it follows and write them up for whoever just inherited it. Reads the stated rule out of agent instructions first, infers the observed pattern from the tests second, and treats the gap between them as the finding — with an explicit note on what it could and could not enumerate. Then asks what to do about it: document the conventions, turn them into agent instructions so mabl's own agents follow them, or both. Where nothing is written down, it proposes a starting set from what's already there rather than reporting nothing. |
