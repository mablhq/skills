...
| Every list carries an explicit `--limit`; the 10-result cap stated once; the no-`--output` commands never shown carrying it; and the two prose-on-stdout commands (`credentials list`, `branches create`) shown **with** their required `sed` | **2** | C |
